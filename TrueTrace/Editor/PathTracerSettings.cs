...
Toggle DIToggle;
Toggle TemporalDIToggle;
Toggle SpatialDIToggle;
Toggle GIToggle;
...
Slider AperatureSlider;
        ...
        public void CreateGUI() {
           ...
           ReSTIRDI = RayMaster.LocalTTSettings.UseReSTIRDI;
           DITemporal = RayMaster.LocalTTSettings.UseReSTIRDITemporal;
           DISpatial = RayMaster.LocalTTSettings.UseReSTIRDISpatial;
           ...
           DIToggle = new Toggle() {value = ReSTIRDI, text = "Use ReSTIR DI"};
           VisualElement DIFoldout = new VisualElement() {};
           Box EnclosingDI = new Box();
               Box TemporalDI = new Box();
                   TemporalDI.style.flexDirection = FlexDirection.Row;
                   TemporalDIToggle = new Toggle() {value = DITemporal, text = "Enable Temporal"};
                   TemporalDI.Add(TemporalDIToggle);
               EnclosingDI.Add(TemporalDI);
               Box SpatialDI = new Box();
                   SpatialDI.style.flexDirection = FlexDirection.Row;
                   SpatialDIToggle = new Toggle() {value = DISpatial, text = "Enable Spatial"};
                   SpatialDIToggle.RegisterValueChangedCallback(evt => {DISpatial = evt.newValue; RayMaster.LocalTTSettings.UseReSTIRDISpatial = DISpatial;});
                   SpatialDI.Add(SpatialDIToggle);
               EnclosingDI.Add(SpatialDI);
           DIFoldout.Add(EnclosingDI);
           MainSource.Add(DIToggle);
           DIToggle.RegisterValueChangedCallback(evt => {ReSTIRDI = evt.newValue; RayMaster.LocalTTSettings.UseReSTIRDI = ReSTIRDI;if(evt.newValue) MainSource.Insert(MainSource.IndexOf(DIToggle) + 1, DIFoldout); else MainSource.Remove(DIFoldout);});
           if(ReSTIRDI) MainSource.Add(DIFoldout);
